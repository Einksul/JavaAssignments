...
public class MindStoneNeighborNeurons {
    ...
    public static void main (String [] args) {
        ...
        SNode[] neurons = new SNode[vertices];


        for(int i = 0 ; i < vertices; i++)
        {
            neurons[i] = new SNode(StdIn.readString(), null);
        }
        ...
        {
            int startindex = find(neurons, StdIn.readString());
            String end = StdIn.readString();
            neurons[startindex].add(new SNode(end,null));
        }
        ...
        for(int i = 0; i < vertices; i++)
        {
            if(neurons[i].getsize() == 0)
            {
                mindStone = i;
            }
        }
        
        String target = neurons[mindStone].getNeuron();
        for(int i = 0; i < vertices; i++)
        {
            if(neurons[i].find(target) == 1)
            {
                StdOut.println(neurons[i].getNeuron());
            }
        }

    }
    public static int find(SNode[] neurons, String Target)
    {
        int index;
        for(int i = 0 ; i < neurons.length; i++)
        {
            if(neurons[i].getNeuron().equals(Target))
            {
                index = i; 
                return index;
            }
