...
public class Board {
    ...
    public void addRandomTile() {
        ...
        int choosenSpace = StdRandom.uniform(0,numOpenSpaces);
        double tileChance = Math.random();
        int tileValue = 0;
        if(tileChance < 0.1) tileValue = 4;
        else tileValue = 2;
        gameBoard[openSpaces.get(choosenSpace).getRow()][openSpaces.get(choosenSpace).getCol()] = tileValue;
    ...
    public void swipeLeft() {
      ...
      {
            rowCord = openSpaces.get(i).getRow();
            colCord = openSpaces.get(i).getCol();
            for(int j = 1; j < 4; j++)
            {
                if(gameBoard[rowCord][j] != 0 && j > colCord)
                {
                    gameBoard[rowCord][colCord] = gameBoard[rowCord][j];
                    colCord = j;
                    gameBoard[rowCord][j] = 0;
