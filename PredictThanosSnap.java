...
public class PredictThanosSnap {
    ...
    public static void main (String[] args) {
        ...
        boolean[] invalid = new boolean[vertices];
        for(int i = 0; i < vertices; i++)
        {
            if(StdRandom.uniform() <= 0.5)
            {
        ...
        StdOut.print(connected(adj, invalid));




    }
    public static void isPath(int[][] adj, int vertex,boolean[] invalid)
    {
        invalid[vertex]=true;
        for(int i = 0; i < adj.length; i++)
        {
            if(adj[i][vertex] == 1 && !invalid[i])
            {
                isPath(adj, i, invalid);
            }
        }
        
    }
    public static void delete(int[][] adj, int vertex)
    {
        for(int i = 0; i < adj.length; i++)
        {
            adj[i][vertex]=0;
    ...
    public static boolean connected(int[][] adj, boolean[] invalid)
    {
        int vertex = 0;
        for(int i = 0; i < adj.length; i++)
        {
